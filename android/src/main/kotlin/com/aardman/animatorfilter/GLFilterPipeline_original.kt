...
class GLFilterPipeline_original(private val outSurface: Surface, private val textureWidth:Int, private  val textureHeight:Int) {
    ...
    private fun setupFilter() {
        this.filterProgram = setupShaderProgram(VertexShaderSource, gaussianShader)
    ...
    private fun setupTestQuadShader() {
        this.filterProgram = setupShaderProgram(VertexShaderSource, DebugFragmentShader)

        // ... other specific setups like uniforms
        // Get vertex shader attributes
        this.attributes["debug_texCoord"] = GLES30.glGetAttribLocation(this.filterProgram, "a_texCoord")
        // Find uniforms
        this.uniforms["u_flipY"] = GLES30.glGetUniformLocation(this.filterProgram, "u_flipY")

        //Enable related attributes (might be in a more generic location, but this sequence is required
        GLES30.glEnableVertexAttribArray(this.attributes["debug_texCoord"]!!)
        // Describe how to pull data out of the buffer, take 2 items per iteration (x and y)
